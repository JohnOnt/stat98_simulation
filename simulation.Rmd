...
## Trying to fix the fucking vizs
```{r}
xyz <-
  ci_data %>%  mutate(name = factor(
    ci_id,
    levels = c(
      'smooth boot 100',
      'smooth boot 30',
...
xyz$Method <- factor(xyz$Method, levels = c("t-based", "Percentile Bootstrap", "Standard Bootstrap", "BCA", "Smooth Bootstrap"))

ggplot(data = xyz, aes(x = ci_bounds, y = name, 
                       group = ci_id, color = Method)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = mean, color = "red") + # draw vertical line
