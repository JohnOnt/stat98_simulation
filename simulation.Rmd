...
print_coverage <- function(data) {
  vec <- c()
  for (i in 1:ncol(normies_coverage)) {
     vec <- c(vec, mean(data[,i]))
  }
  
  tmp.df <- data.frame(matrix(ncol=1, nrow=15))
  colnames(tmp.df) <- c('coverage probability')
...
# Print Coverage rate
print_coverage(normies_coverage)  
...
t.test(normies10, conf.level = 0.95)
n <- length(normies10)
T9 <- StudentsT(df=n-1)
mean(normies10) + quantile(T9, 0.05 / 2) * sd(normies10) / sqrt(n)
mean(normies10) + quantile(T9, 1 - 0.05 / 2) * sd(normies10) / sqrt(n)

```
```{r}
geomies10 = rgeom(10, 0.7)

t.test(geomies10, conf.level = 0.95)$conf.int[2]
n <- length(geomies10)
T9 <- StudentsT(df=n-1)
mean(geomies10) + quantile(T9, 0.05 / 2) * sd(geomies10) / sqrt(n)
mean(geomies10) + quantile(T9, 1 - 0.05 / 2) * sd(geomies10) / sqrt(n)
...
#i = 3
  ...
  n <- length(geomies10)
  Ts <- StudentsT(df=n-1)
  lower10t <- mean(geomies10) + quantile(Ts, 0.05 / 2) * sd(geomies10) / sqrt(n)
  #t.test(geomies10, conf.level = 0.95, )$conf.int[1]
  upper10t <- mean(geomies10) + quantile(Ts, 1 - 0.05 / 2) * sd(geomies10) / sqrt(n)
  #t.test(geomies10, conf.level = 0.95)$conf.int[2]
  
  # Do the same for the rest of the n sample sizes
  n <- length(geomies30)
  Ts <- StudentsT(df=n-1)
  lower30t <- mean(geomies30) + quantile(Ts, 0.05 / 2) * sd(geomies30) / sqrt(n)
  #t.test(geomies30, conf.level = 0.95)$conf.int[1]
  upper30t <- mean(geomies30) + quantile(Ts, 1 - 0.05 / 2) * sd(geomies30) / sqrt(n)
    #t.test(geomies30, conf.level = 0.95)$conf.int[2]
  
  n <- length(geomies30)
  Ts <- StudentsT(df=n-1)
  lower100t <- mean(geomies100) + quantile(Ts, 0.05 / 2) * sd(geomies100) / sqrt(n)
    #t.test(geomies100, conf.level = 0.95)$conf.int[1]
  upper100t <- mean(geomies100) + quantile(Ts, 1 - 0.05 / 2) * sd(geomies100) / sqrt(n)
    #t.test(geomies100, conf.level = 0.95)$conf.int[2]
...
# Print Coverage rate
print_coverage(geomies_coverage)
```
